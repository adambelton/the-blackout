 ...
 * resequenced after applying, the K6.2 cursor-poisoning incident,
 * etc.), and the next `migrate` then either errors with "already
 * exists" or silently no-ops past the tables it needed to create.
 *
 * This check is the structural fix. It asserts:
 *   1. Every table defined in schema.ts exists in the DB.
 *   2. Every applied-migration row matches its journal entry by hash.
 *   3. Cursor count equals journal count (post-migrate, no pending).
 *
