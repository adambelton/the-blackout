 ...
 * (if a seed script exists). The migration system becomes the source
 * of truth again in one command. Safer than surgical interventions,
 * because the failure mode of "the cursor said X but the schema was
 * at Y" can't recur from a fresh start.
 *
 * Safety: refuses to run unless DATABASE_URL points at a local DB
 * whose name contains `blackout` AND host is loopback. Prod DBs
 * (Neon, Fly Postgres) will fail this check by hostname.
 */
import "../env.js";
...
const looksLocal = dbName.includes("blackout") && isLoopback;
...
if (!looksLocal) {
  ...
  console.error(
    `  This script wipes the database. Only runs against loopback hosts with "blackout" in the db name.`,
  );
  process.exit(1);
}
...
const seedPath = resolve(serverRoot, "src/db/seed.ts");
...
console.log(`[db:reset] OK — ${dbName} reset, migrated${existsSync(seedPath) ? ", and seeded" : ""}.`);
